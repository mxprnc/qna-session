...
### 🌟 주요 기능
1. **순차적 Q&A 기록 및 자동 보관**:
   - 사용자가 '종료'하겠다는 의사를 표현하기 전까지 질문1:답변, 질문2:답변, ..., 질문n:답변 과정을 무한히 반복하여 Q&A 기록을 보관합니다.
   - `session_manager.py` 스크립트를 통해 세션 초기화(`init`), 내용 추가(`append`), 세션 종료 및 컴파일(`finalize`)의 전체 라이프사이클을 관리합니다.
2. **HTML 변환 타입 선택 기능**:
   - Q&A 세션이 완료되어 보관된 마크다운 문서를 HTML로 변환할 때, 용도에 따라 **overall** 타입과 **raw** 타입을 선택하여 변환할 수 있습니다:
     - **overall** 타입: 사이드바 테이블 오브 콘텐츠(TOC)와 접고 펼치기 기능이 탑재된 구조화된 종합 보고서 형태.
     - **raw** 타입: 검색, 복사 및 PrismJS 코드 하이라이팅이 적용된 순차 상세 카드 레이아웃 형태.
3. **유연한 데이터 익스포트 및 중복 방지**:
...
- **세션 시작**: `"Q&A 세션 시작해줘"` 또는 `"qna-session 시작"` (사용자가 '종료' 의사를 표현할 때까지 질문1:답변, 질문2:답변... 순으로 반복하여 기록 보관)
- **세션 종료**: 세션 진행 중 `"종료"` 또는 `"끝"` 입력
- **HTML 변환 타입 지정**: 종료 후 저장된 기록 문서를 HTML로 변환할 때, 필요에 따라 `overall` 또는 `raw` 타입을 명시하여 익스포트를 요청할 수 있습니다.
- **익스포트 요청 예시**:
